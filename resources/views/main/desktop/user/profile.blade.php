...
@section('content')
            ...
            <section class="section profile-user">
                ...
                <div class="global_select" >
                    <select  class="select2 js-states form-control" name="country_id" required>
                        @foreach (Modules\Entity\Model\LibCountry\LibCountry::orderBy('name')->get() as $c)
                            <option data-code="{{$c->code}}" value="{{ $c->id }}" {{ $student_data->country_id == $c->id ? 'selected' : '' }}>{{ $c->name }}
							</option>
                        @endforeach
                    </select>
                </div>
	...
	<script>
	//определим код выбранной страны
	var code = $('select[name=country_id] option:selected').attr('data-code');
	
	$('input[name=country_code]').val(code);
	
	if(code){
	$('.global_input-phone p').text(code);
	}else{
