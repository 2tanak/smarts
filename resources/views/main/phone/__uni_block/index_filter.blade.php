<section class="section filter">
    <form action="">
        ...
        <div class="global_select mb-3">
            <select class="select2 js_flag_select  js-states form-control" name="country_id" id="country_id">
                <option value="" >@lang('front_main.note_selected')</option>
                @foreach ($filter_lib['country'] as $c)
                    <option value="{{ $c->id }}" data-img="{{ $c->photo ? env('FILE_SERVER_URL').'/'.$c->photo : '' }}" {{ $request->country_id == $c->id ? 'selected' : '' }}>{{ $c->name }}</option>
                @endforeach
            </select>
        </div>
        <!--
        ...
        <button class="button_white w-100 mt-3">@lang('front_main.filter.submit')</button>
    </form>
</section>
